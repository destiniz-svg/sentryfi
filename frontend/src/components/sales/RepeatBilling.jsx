 ...
 * the books by itself.
 */
export function RepeatBilling({ onClose }) {
  const { companyId, can } = useCompany();
  const toast = useToast();
  const qc = useQueryClient();
  const [adding, setAdding] = useState(false);
  const { data } = useQuery({ queryKey: ["recurring", companyId], queryFn: () => apiClient.get("/recurring").then((r) => r.data.schedules) });
  const refresh = () => {
    for (const k of ["recurring", "sales", "sales-aged", "figures", "attention"]) qc.invalidateQueries({ queryKey: [k, companyId] });
  };

  return (
    <Modal open onClose={onClose} title="Repeat billing" description="Invoices raised on a schedule, on their date, without anyone remembering to.">
      {adding ? (
        <NewSchedule
          ...
          onDone={(r) => {
            refresh();
            setAdding(false);
            toast.success("Schedule saved", r.raised.length ? `${r.raised.length} ${r.raised.length === 1 ? "invoice was" : "invoices were"} due already, and raised.` : "The first invoice is raised on its date.");
          }}
        />
      ) : (
        <>
          {!data ? (
            ...
            <p className="text-[14px] text-[var(--ink-muted)]">Nothing is billed on a schedule yet.</p>
          ) : (
    ...
    <div className="grid gap-3">
      <div className="grid sm:grid-cols-2 gap-3">
        <label className="block">
          <span className="text-sm font-medium block mb-1.5">Customer</span>
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        ...
        <label className="block">
          <span className="text-sm font-medium block mb-1.5">First invoice</span>
      ...
      <div className="flex justify-end gap-2 mt-2">
        ...
        <Button type="button" variant="accent" disabled={busy || !f.customerName.trim() || !f.description.trim() || !f.unitPrice} onClick={save}>
