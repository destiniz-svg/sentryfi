...
async function create(client, { companyId, userId, counterpartyId, name, lines, gstTreatment = "exclusive", every, startsOn, endsOn, postAutomatically = false, projectId }) {
  ...
  if (!p.length) throw new Error("That customer is not in these books.");
  if (projectId) {
  ...
  const { rows } = await client.query(
    ...
    [companyId, counterpartyId, String(name || "").trim() || clean[0].description, JSON.stringify(clean), gstTreatment, every, startsOn, Number(startsOn.slice(8, 10)), endsOn || null, Boolean(postAutomatically), projectId || null, userId]
  );
  return rows[0].id;
}
...
async function runDue(client, { companyId, userId, today }) {
  ...
  for (const r of rows) {
    ...
    for (let i = 0; i < 53 && next <= on && (!r.ends_text || next <= r.ends_text); i++) {
      ...
      raised.push({ scheduleId: r.id, invoiceId: invoice.id, invoiceNo: invoice.invoice_no, on: next, posted: r.post_automatically });
...
async function list(client, { companyId }) {
  const { rows } = await client.query(
    `SELECT r.*, r.next_on::text AS next_text, r.ends_on::text AS ends_text, c.name AS customer,
            (SELECT count(*)::int FROM sales_invoices s WHERE s.recurring_id = r.id) AS raised
       FROM recurring_invoices r JOIN counterparties c ON c.id = r.counterparty_id
      WHERE r.company_id = $1 ORDER BY r.next_on`,
    [companyId]
  );
  return rows.map((r) => {
    const each = r.lines.reduce((a, l) => a + (toLaari(l.unitPrice) * BigInt(Math.round(Number(l.quantity) * 10000)) + 5000n) / 10000n, 0n);
    return {
