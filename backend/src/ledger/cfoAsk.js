...
async function run(client, ctx, name, args = {}) {
  ...
  if (name === "documents") {
    ...
    const owed = invoices
      ...
      : "d.gross_laari - COALESCE((SELECT SUM(p.amount_laari) FROM payment_items p JOIN payment_runs pr ON pr.id = p.run_id AND pr.reversed_at IS NULL WHERE p.bill_id = d.id), 0) + bill_returned(d.id)";
