...
async function figures(client, { companyId, today }) {
  ...
  const { rows: bills } = await client.query(
    `SELECT b.id, b.bill_no, c.name, COALESCE(b.due_date, b.issue_date + 30)::text AS due,
            b.gross_laari - COALESCE((SELECT SUM(p.amount_laari) FROM payment_items p JOIN payment_runs pr ON pr.id = p.run_id AND pr.reversed_at IS NULL WHERE p.bill_id = b.id), 0) + bill_returned(b.id) AS owed
