...
**Now:** Sprint 2, item 6 (fixed assets and year end). **Sprint 1 done 23 September 2026.** Item 5: the desk shell, a grouped rail (Needs you, Money, The books; Settings and the person at the foot; icons only at tablet width), a breadcrumb bar read from `lib/sections.js`, and one yellow action per page, checked by `tools/desk.js`. Item 4 done 23 September 2026: the expense manager reads Home · Tin · Send · Owed back · Me; Owed back shows what the holder paid from their own pocket. A "paid from the tin" choice on Send is not built: spending from the tin is on Tin. Item 3 done 23 September 2026: bank lines on a phone ask one question at a time with a bar, the remembered answer in yellow, Leave for later quiet. Items 1 and 2 done 23 September 2026: tab bar, Record sheet, More, offline line; Home (cash card, thirty-day line, runway, cash per place, Needs you) and Money (Bills | Invoices, sticky total, day headers, money rows, swipe to put in the books). Parts in `components/mobile/parts.jsx`; the pill is `Badge`, the undo strip `UndoContext`. Checked by `tools/tabbar.js`.
