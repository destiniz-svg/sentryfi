...
27. **The owner's stock snapshot**: what came in, what is where, what it is worth, what moved, what looks wrong. Every figure opens onto its moves. Value by place as at any date. *Done 26 Sep 2026:* Inventory > Stock (`/inventory`, read roles only) above Items. As at any date with a week or month: deliveries by supplier and place; each place's quantity and value (a place's share of the item's company-wide average; the last place takes the rounding, so places add up to the item), goods on the way as a place of their own; the total against the Stock account (1350) on that date; sold net of returns, used on jobs, transfers; and what looks wrong: short counts and arrivals, on the way over 7 days, below nothing at a place, no movement for 90 days, at or below reorder, and books that disagree. Every figure opens its moves (`GET /stock/moves`). Nothing stored: it is sums over the moves (tools/stock-snapshot.js). Limits of 7 and 90 days are fixed for now.
