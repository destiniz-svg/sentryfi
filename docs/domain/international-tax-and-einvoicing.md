...
**A note on the Maldives, corrected 23 September 2026.** This file first asserted that MIRA charges GST at the border on imported goods. That is contradicted by the Maldives reference in this folder, which read MIRA's own pages (through archived copies, since the live site refuses automated readers) and found the tax charged on goods and services *supplied in the Maldives*, with input tax expressly barred where the good or service is not supplied here; goods crossing the border meet import duty under the Export Import Act, not GST. The owner of this product, who imports for a living, says the same. Treat the border-charge passages below as describing the *other* model — the one most countries use — and see `to-confirm.md` item 1 for the single document that settles it beyond argument.
...
**Model B — border charge with no deferment** (the model this file first attributed to the Maldives; see the correction above. It applies in various smaller jurisdictions that charge at the border and offer no postponed accounting).
- Import GST is charged at the border on CIF plus duty, and recovered only through the periodic return.
