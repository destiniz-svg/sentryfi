...
`bills.place_id` and `order_deliveries.place_id` name the `stock_places` row the goods came into (none is the main store); a place has a `kind`, an `in_charge` user (a member, checked in code) and, for a site, a `project_id`.
